...
export default function App() {
  ...
  // Build grid with sponsored at positions #5 (idx 4) and #12 (idx 11)
  ...
  const buildDesktopGrid = (): DesktopGridItem[] => {
    ...
    // Slot #12 (index 11) — only if we have enough organic
    if (items.length >= 14) {
      const slot12: DesktopGridItem = topCampaign
        ? { kind: 'campaign', camp: topCampaign }
        : topHouseAd
          ? { kind: 'house', ad: topHouseAd }
          : { kind: 'organic', dest: organic[13] };
      items[13] = slot12;
    }
    return items;
  };
  ...
  const buildEventCarousel = (): EventCarouselItem[] => {
    ...
    // Add to the end instead of splicing
    organic.push({ kind: 'sponsored', camp: nativeCampaign });
    console.log('DEBUG buildEventCarousel pushed sponsored. New length:', organic.length);
