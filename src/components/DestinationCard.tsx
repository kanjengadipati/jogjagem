...
export default function DestinationCard({ destination, onExplore, onToggleSave, isSaved, className = '' }: DestinationCardProps) {
  ...
  return (
    ...
    >
      ...
      {/* Top-Left Badge */}
      <div className="absolute top-2 left-2 sm:top-4 sm:left-4 bg-black/40 backdrop-blur-md border border-white/10 px-2 py-0.5 sm:px-3 sm:py-1 rounded-full text-[8px] sm:text-[10px] font-sans font-semibold uppercase tracking-[0.08em] text-white">
        {badgeText}
      </div>
